...
def apply_filters(frame: pd.DataFrame, role_type: str, location: str, company: str, min_score: float) -> pd.DataFrame:
    ...
    if role_type:
        filtered = filtered[
            ...
            | filtered.get("description", pd.Series("", index=filtered.index)).str.contains(role_type, case=False, na=False)
        ]
    if location:
...
for _, row in filtered.head(50).iterrows():
    ...
    with st.expander(label):
        ...
        with detail_cols[1]:
            ...
            st.write(f"**Age:** {row['age'] or 'Unknown'}")
            if str(row.get("salary", "")):
